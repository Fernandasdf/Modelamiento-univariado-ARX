...
)
{
  ...
  results <- list(models = list(model), stats = stats, "model"=model) 
...
eval.model <- function(
  ...
  # pred <- predict(model, signal.test$folded.signal)
  # pred <- round(pred, 4)
  # # print(model)
  # test.cor <- cor(pred, signal.test$folded.signal["CBFV"])
  # test.cor <- round(test.cor, 4)
  # print(test.cor)
  ...
  pred <- response.ARX.model
  pred <- round(pred, 3)
  test.cor <- cor(response.ARX.model,y)
  ...
  )
{
  ...
  if(step.test==FALSE){
    ...
    colnames(stats) <- c("MABP","CBFV","nu","cost","train.cor","test.cor","fold","stepTest")
